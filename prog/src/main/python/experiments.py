...
def experiment6():
    outputs = 3
    pMut = 0.1
    trainingSet = list(range(2**outputs))
    shuffle(trainingSet)
    print(trainingSet)
    ...
    def symbSearch(x):
        ...
        sr.setMiniMaxi(True);
        return sr
    solutionValues, solutionStrings = runsWithStrings(list(map(symbSearch, range(runsCount))))
...
def callBacksRunsNondominance(searchingCallBacks, criterions):
    for callBack in searchingCallBacks:
        criterionsValues, solutionValues, solutionStrings = multicriterialRuns(list(map(callBack, range(runsCount))), criterions)
...
def experiment4():
    ...
    for algorithm in ["Vega", "Spea"]:
        print(algorithm)
        results = {}
        for pMut in [0.01, 0.05]+[x/10. for x in range(10)]:
            for pCross in [0.0]:
            #for pCross in [0.01, 0.05]+[x/10. for x in xrange(10)]:
                ...
                criterionsValues, solutionValues, solutionStrings = multicriterialRuns(list(map(func, range(runsCount))), criterions)
                results[(pMut, pCross)] = criterionsValues, solutionValues

        #prumerny jedinec
        averageSolution = [0 for i in range(CRITERIONS_COUNT)]
        for criterion in criterions:
            averageSolution[criterion.ordinal] =  numpy.floor(10*numpy.mean(numpy.concatenate([i[0][criterion] for i in results.values()])))/10
        ...
        max = (0, 0)
        maxCount = 0
        for (pMut, pCross), (criterionsValues, solutionValues) in results.items():
            # a nezalezi z kolika nBestGenomes (prave ze by se meli nashlukovat ti dobri, kdyz jich u SPEA bude min)
            aboveAverage = [solution for solution in solutionValues if weakDominatingOver(solution, averageSolution, criterions)]
        ...
        keys = sorted([pMut_pCross_count for pMut_pCross_count in iter(aboveAverageCounts.items()) if pMut_pCross_count[1] > 0])
        print(keys)
        if len(keys) > 0:
            figure()
            ...
            ylabel("Count")
            xticks(ind + width/2.0, ["(%.2g,%.1f)"%(x[0][0], x[0][1]) for x in keys])
            bar(ind, [aboveAverageCounts[key[0]] for key in keys], width)
    ...
    for genomeType in [ChromozomeType.CGP, ChromozomeType.SOFTWARE_IMPL]:
        ...
        for criterion in allCriterions:
            ...
            for pMut in [0.01, 0.05]+[x/10. for x in range(10)]:
                ...
                results[pMut] = runs(list(map(func, range(runsCount))))
            ...
            randomSearchResult = runs(list(map(func, range(runsCount))))

            stats = []
            for pMut, values in results.items():
        ...
        for criterion, i in criterionBests.items():
...
def experiment2():
    ...
    for criterion in allCriterions:
        ...
        for algorithm in algorithms:
            ...
            list = list(map(func, range(runsCount)))
            result = runs(list)
    ...
    for algorithm in algorithms:
        ...
        for criterion, data in algoritmResults[algorithm].items():
            i = runsStats(data)
...
def crossAndMutationBoxPlot(results, criterion):
    ...
    xlabel(criterion)
    yticks(arange(len(results)), ["(%.2g,%.1f)"%(x[0], x[1]) for x in sortedResultsKeys])
    boxplot([results[x] for x in sortedResultsKeys], vert=0)
...
def experiment1():
    for criterion in allCriterions:
        ...
        for pMut in [x/25. for x in range(5)] :
            for pCross in [x/25. for x in range(5)]:
                results[(pMut, pCross)] = runs([Searching("Ga", genome, popSize, generations, pMut, pCross, False) for x in range(runsCount)])

        #nakresleni grafu...
        boxplotFileName = crossAndMutationBoxPlot(results, criterion)

        stats = []
        for (pMut, pCross), values in results.items():
        ...
        resultOfRandomSearch = runs([Searching("Random", genome, popSize, generations) for x in range(runsCount)])
        figure()
        xlabel("Algoritmus")
        ylabel(criterion)
